    ...
    const _handleSubmitAgree = async() =>{
        ...
        .then(data=>{
            if (data.message === 'connect success') {
                editorApi.postUpdate2Server(cookiesUSID, postTitle, postModule, commonFiles, queryValues.BomNo, queryValues.Category, queryValues.Pr, queryValues.PostVal, postMetaData)
                .then(data=>{
                    if(data.message==='success'){
